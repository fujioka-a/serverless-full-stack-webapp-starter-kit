...
このキットは、拡張性よりも「最初にコピーして動かし、全体を読み切れること」を優先したデフォルトを選んでいます。

| 選定 | このキットでの考え方 |
|------|----------------------|
| `prisma db push` をデフォルト採用 | スターターキットでは、厳密な migration 履歴管理よりも schema 変更を素早く反映できることを優先しています。履歴管理が必要になったら `prisma migrate` に切り替えられます。 |
| NAT Gateway ではなく NAT Instance | 常時コストを抑えるためです。可用性や運用の複雑さより、まず低コストで始めやすいことを重視しています。 |
| 非同期ジョブは単一 Lambda に集約 | 関数を用途ごとに細かく分けるより、デプロイと全体理解を簡単にすることを優先しています。エントリーポイントは CDK の `cmd` で切り替えます。 |
| 認証保護は `proxy.ts` で実装 | Next.js middleware ではなく Lambda ハンドラ内で処理することで、認証時の余計な cold start コストを避けやすくしています。 |
| `output: 'standalone'` を使用 | Next.js アプリを Lambda 用の Docker イメージとして配備する前提に合わせた構成です。 |
| Lambda Web Adapter を使用 | CloudFront と Lambda Function URL の組み合わせで、レスポンスストリーミングを使えるようにするためです。 |
